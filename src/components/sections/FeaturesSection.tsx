      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        <div className="grid grid-cols-1 gap-8">
          {features.content.map((cards) => {
            const Icon = Icons[cards.icon as keyof typeof Icons || "blank"]; // Corrected for TypeScript
            ...
            return (
              ...
              >
                <div className="flex">
                  <Icon className="h-[6rem] w-[6rem] text-sky-900" />
                </div>
                <div className="flex-1">
                  <p className="md:text-4xl text-2xl font-semibold">{cards.text}</p>
                  <p className="font-light text-muted-foreground md:text-lg">{cards.subtext}</p>
                </div>
              </div>
            )
          })}
